        ...
        private void Impression_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (con.State == ConnectionState.Closed)
                    con.Open();
                com.CommandText = "select CodeLivr from Livraison";
                read = com.ExecuteReader();
                if (read.HasRows)
                {
                    t.Load(read);
                    comboBox1.DataSource = t;
                    comboBox1.ValueMember = "CodeLivr";
                }
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            if (comboBox1.Text != "")
            {
                try
                {
                    ...
                    data = new SqlDataAdapter("select * from Livraison where CodeLivr=" + comboBox1.Text, con);
                    data.Fill(set, "Livraison");
                    data = new SqlDataAdapter("select cha.* from Charge_Commande as cha inner join Chargement as ch on ch.CodeChargement=cha.CodeChargement inner Join Livraison as l on l.CodeChargement=ch.CodeChargement where l.CodeLivr=" + comboBox1.Text, con);
                    data.Fill(set, "Charge_Commande");
                    data = new SqlDataAdapter("select ch.* from Chargement as ch  inner Join Livraison as l on l.CodeChargement=ch.CodeChargement where l.CodeLivr=" + comboBox1.Text, con);
                    data.Fill(set, "Chargement");
                    data = new SqlDataAdapter("select chau.* from chauffeur as chau  inner join Chargement as ch on chau.CodeChauffeur=ch.CodeChauf  inner Join Livraison as l on l.CodeChargement=ch.CodeChargement where l.CodeLivr=" + comboBox1.Text, con);
                    data.Fill(set, "Chauffeur");
                    data = new SqlDataAdapter("select C.* from Commande as C inner Join Charge_Commande as char on C.Codecommande=char.CodeCommande  inner join Chargement as ch on ch.CodeChargement=char.CodeChargement  inner Join Livraison as l on l.CodeChargement=ch.CodeChargement where l.CodeLivr=" + comboBox1.Text, con);
                    data.Fill(set, "Commande");
                    data = new SqlDataAdapter("select Cl.* from Client as Cl inner Join Commande as C on C.CodeClient=Cl.Codeclient inner Join Charge_Commande as char on C.Codecommande=char.CodeCommande  inner join Chargement as ch on ch.CodeChargement=char.CodeChargement  inner Join Livraison as l on l.CodeChargement=ch.CodeChargement where l.CodeLivr=" + comboBox1.Text, con);
                    data.Fill(set, "Client");
                    BonLivraison b = new BonLivraison();
                    b.SetDataSource(set);
                    Report_Viewer_Form r = new Report_Viewer_Form();
                    r.Text = "Bon de Livraison N° " + comboBox1.Text;
            ...
            else MessageBox.Show("Choisissez une Livraison pour L'Imprimer", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
