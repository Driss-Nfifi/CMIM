...
today = mm + '/' + dd + '/' + yyyy + ' ' + hh + ':' + MMM;
  ...
  if(!this.state.update && this.state.add)
  {
      ...
      .then(res => {
        if (res.status == 201)
        {
          ...
          this.setState({
            ...
            regularisationID:res.data.regularisationID
          });
          ...
          if(res.data["listDesRégularisation"] != null) {
          console.log(res.data["listDesRégularisation"]);
     for(var i = 0; i < res.data["listDesRégularisation"].length; i++) {
       tabs[i] = [];
       tabs[i][0] = res.data["listDesRégularisation"][i]["employeeMatricule"];
       tabs[i][1] = res.data["listDesRégularisation"][i]["total"] + " DHS";
     }
     console.log("jsPDF: ")
     console.log(tabs);
   ...
   /*  var splitTitle = pdf.splitTextToSize("Impression de bordereau", 180);
     ...
     pdf.autoTable({
            head:[["Code Employé", "Montant total de régularisation"]],
             body:tabs
                 });
